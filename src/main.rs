...
fn collect_font_files(paths: &[&String]) -> anyhow::Result<Vec<String>> {
    ...
    for path_str in paths {
        ...
        if path.is_dir() {
            // Read directory and collect all files
            for entry in fs::read_dir(path)? {
                let entry = entry?;
                let entry_path = entry.path();
                if entry_path.is_file() {
                    // Check if file has common font extensions
    ...
    if font_files.is_empty() {
        return Err(anyhow::anyhow!("No font files found in the specified paths"));
    }
...
fn main() -> anyhow::Result<()> {
    ...
    let font_files = collect_font_files(&font_paths)?;
    ...
    println!("Successfully generated .mobileconfig file: {}", output_path.display());
